        ...
        if ('Test and Analysis' in config.targetStages)
            stage('Test and Analysis') {
                parallel(
                        ...
                        ,
                        'Static Analysis': {
                            sh "${mvnCmd} org.jacoco:jacoco-maven-plugin:prepare-agent jacoco:report sonar:sonar -Dsonar.host.url=http://sonarqube:9000 -DskipTests=true sonar.jacoco.reportMissing.force.zero=true"
                        }
                )
            }
